...
import com.udit.aijiabao.utils.ui.MyGifView;
...
public class Practice4Adapter extends PagerAdapter {

    Practice4Activity mContext;
    // 传递过来的页面view的集合
    List<View> viewItems;
    // 每个item的页面view
    View convertView;
    ...
    public Practice4Adapter(Practice4Activity context, List<View> viewItems, List<Question>
            dataItems) {
        mContext = context;
        this.viewItems = viewItems;
        this.dataItems = dataItems;

        dbManager = x.getDb(((MyApplication) mContext.getApplicationContext()).getDaoConfig4());
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        mposition=position+Practice4Activity.pos;
        ...
        holder.gif = (MyGifView) convertView.findViewById(R.id.activity_prepare_test_gif);
        ...
        holder.totalText.setText(position + Practice4Activity.pos + "/" + Practice4Activity.allnum);
        //判断是否收藏，UI变换
        ...
        if (isExist(FileUtils.getproject4Path() + "/" + dataItems.get(position).getTopicId() + ".gif")) {
            Log.e("bum","gif:存在"+dataItems.get(position).getTopicId() + ".gif");
            holder.gif.setVisibility(View.VISIBLE);
            holder.img.setVisibility(View.GONE);
            holder.gif.setMovie(getLoacalGif(FileUtils.getproject4Path() + "/" + dataItems.get(position).getTopicId() + ".gif"));
            holder.gif.setClickable(true);
            holder.gif.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    // 设置暂停
                    if (holder.gif.isPaused()) {
        ...
        }else if (isExist(FileUtils.getproject4Path() + "/" + dataItems.get(position).getTopicId() + ".jpg")) {
            Log.e("bum","jpg:存在"+dataItems.get(position).getTopicId() + ".jpg");
            holder.img.setVisibility(View.VISIBLE);
            holder.gif.setVisibility(View.GONE);
            bitmap = getLoacalBitmap(FileUtils.getproject4Path() + "/" + dataItems.get(position).getTopicId() + ".jpg");
            holder.img.setImageBitmap(bitmap);
        }else {
            Log.e("bum","都不存在：topicID："+dataItems.get(position).getTopicId());
            holder.img.setVisibility(View.GONE);
            holder.gif.setVisibility(View.GONE);
        }
        if (dataItems.get(position).getCorrectAnswer().length() == 1) {
            ...
            holder.multi_choice.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    savePosition(Practice4Activity.a);
                    ...
                    holder.multi_choice.setBackgroundResource(R.drawable.grayblack_bg);
                    holder.multi_choice.setClickable(false);
                    //提示
        ...
        */
        ...
        if (mposition == viewItems.size()) {
            if (position+ Practice4Activity.pos== Practice4Activity.allnum){
    ...
    private void multi_drawable(ViewHolder holder) {
        if (multi_answer[0]=="0"&&multi_answer[1]=="0"&&multi_answer[2]=="0"&&multi_answer[3]=="0"){
            holder.multi_choice.setBackgroundResource(R.drawable.grayblack_bg);
            holder.multi_choice.setClickable(false);
        }else {
            holder.multi_choice.setBackgroundResource(R.drawable.btn_selector_blue);
            holder.multi_choice.setClickable(true);
        }
    }
    //判断是否收藏
        ...
        switch (type){
            ...
            default:
                switch (type){
                    ...
                    case "390":
                        try {Log.e("bum","mposition="+mposition);
                            ChapterName chapterName=dbManager.findById(ChapterName.class,8);
                            chapterName.setPosition(mposition);
                            dbManager.update(chapterName);
                        } catch (DbException e) {
                            e.printStackTrace();
                    ...
                    case "400":
                        try {Log.e("bum","mposition="+mposition);
                            ChapterName chapterName=dbManager.findById(ChapterName.class,9);
                            chapterName.setPosition(mposition);
                            dbManager.update(chapterName);
                        } catch (DbException e) {
                            e.printStackTrace();
                    ...
                    case "410":
                        try {Log.e("bum","mposition="+mposition);
                            ChapterName chapterName=dbManager.findById(ChapterName.class,10);
                            chapterName.setPosition(mposition);
                            dbManager.update(chapterName);
                        } catch (DbException e) {
                            e.printStackTrace();
                    ...
                    case "420":
                        try {Log.e("bum","mposition="+mposition);
                            ChapterName chapterName=dbManager.findById(ChapterName.class,11);
                            chapterName.setPosition(mposition);
                            dbManager.update(chapterName);
                        } catch (DbException e) {
                            e.printStackTrace();
                    ...
                    case "430":
                        try {Log.e("bum","mposition="+mposition);
                            ChapterName chapterName=dbManager.findById(ChapterName.class,12);
                            chapterName.setPosition(mposition);
                            dbManager.update(chapterName);
                        } catch (DbException e) {
                            e.printStackTrace();
                    ...
                    case "440":
                        try {
                            ChapterName chapterName=dbManager.findById(ChapterName.class,13);
                            Log.e("bum","chapterName="+chapterName.toString());
                            chapterName.setPosition(mposition);
                            Log.e("bum_mposition=",""+mposition);
                            dbManager.update(chapterName);
                    ...
                    case "450":
                        try {Log.e("bum","mposition="+mposition);
                            ChapterName chapterName=dbManager.findById(ChapterName.class,14);
                            chapterName.setPosition(mposition);
                            dbManager.update(chapterName);
                        } catch (DbException e) {
                            e.printStackTrace();
    ...
    class LinearOnClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (mIsNext){
                if (mposition== Practice4Activity.allnum&&mPosition==viewItems.size()){
                    mContext.finish();
                }else if (mPosition == Practice4Activity.num) {
                    if (mposition== Practice4Activity.allnum){
                        mContext.finish();
                    }else {
                        Practice4Activity.pos= Practice4Activity.pos+ Practice4Activity.num;
                        Log.e("bum","Practice4Activity.pos="+ Practice4Activity.pos);
                        Practice4Activity.isNext=true;
                        mContext.initData();
                    }
                }else mContext.setCurrentView(mPosition);
            } else {
                if (mPosition == -1) {
                    if (Practice4Activity.pos==1){
                        Toast.makeText(mContext, "已经是第一页", Toast.LENGTH_SHORT).show();
                        return;
                    }else {
                        if (Practice4Activity.pos< Practice4Activity.num){
                            Practice4Activity.po= Practice4Activity.pos;
                            Practice4Activity.pos=1;
                            Practice4Activity.isNext=false;
                            mContext.initData();
                        }else {
                            Practice4Activity.pos= Practice4Activity.pos- Practice4Activity.num;
                            Practice4Activity.isNext=false;
    ...
    public static Movie getLoacalGif(String url) {
        try {
            FileInputStream fis = new FileInputStream(url);
            return Movie.decodeStream(fis);  ///把流转化为Movie图片
