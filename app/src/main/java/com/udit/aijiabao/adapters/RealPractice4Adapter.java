...
public class RealPractice4Adapter extends PagerAdapter {

    RealPractice4Activity mContext;
    // 传递过来的页面view的集合
    List<View> viewItems;
    // 每个item的页面view
    View convertView;
    ...
    public RealPractice4Adapter(RealPractice4Activity context, List<View> viewItems, List<RealTest>
            dataItems) {
        mContext = context;
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        if (dataItems.get(position).getCorrectAnswer().length() == 1) {
            ...
            holder.layoutA.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View arg0) {
                    ...
                    multi_drawable(holder);//根据是否有选中对象，改变确认答案按钮的状态（背景和是否可点击）
                }
            });
            holder.layoutB.setOnClickListener(new OnClickListener() {
        ...
        /**
         *读取进度，如果有答案则显示对错,并添加进map，
         * */
        if (dataItems.get(position).getYourAnswer() != null) {
            map.put(position, true);
            if (dataItems.get(position).getCorrectAnswer().length() == 1){
                //单选
                if (dataItems.get(position).getCorrectAnswer().equals(dataItems.get(position).getYourAnswer()))
                {//如果作对，显示对号标记
                ...
                String youranswer=dataItems.get(position).getYourAnswer();
                String[] multi_answer;
                multi_answer=new String[]{youranswer.substring(0,1),youranswer.substring(1,2),youranswer.substring(2,3),youranswer.substring(3,4)};
                String correctAnswer=dataItems.get(position).getCorrectAnswer();
